const apiUrl = 'https://quick-credit-max.herokuapp.com/api/v1/loans?status=approved&repaid=false';
...
const getCurrentLoans = async () => {
  ...
  if (loans.length === 0) {
    loanTable.innerHTML = `<p style='font-size: 1.3em; text-align: center'>There are no current loans</p>`;
  }
  loans.forEach((loan) => {
...
window.addEventListener('load', getCurrentLoans);
